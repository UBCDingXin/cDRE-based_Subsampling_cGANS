...
Functions for training unconditional density-ratio model
...
# training function
def train_dre(trainloader, dre_net, dre_precnn_net, netG, net_y2h, path_to_ckpt=None):
    ...
    if path_to_ckpt is not None and dre_resume_epoch>0:
        ...
        ckpt_fullpath = path_to_ckpt + "/DRE_{}_checkpoint_epoch_{}.pth".format(dre_net_name, dre_resume_epoch)
        checkpoint = torch.load(ckpt_fullpath)
        dre_net.load_state_dict(checkpoint['net_state_dict'])
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])

        #load d_loss and g_loss
        logfile_fullpath = path_to_ckpt + "/DRE_{}_train_loss_epoch_{}.npz".format(dre_net_name, dre_resume_epoch)
    ...
    for epoch in range(dre_resume_epoch, dre_epochs):
        ...
        for batch_idx, (batch_real_images, _) in enumerate(trainloader):
            ...
            batch_fake_labels = np.random.uniform(low=0.0, high=1.0, size=batch_size_curr)
            ...
            DR_real = dre_net(batch_features_real)
            DR_fake = dre_net(batch_features_fake)
        ...
        print("DRE+{}+lambda{}: [epoch {}/{}] [train loss {}] [Time {}]".format(dre_net_name, dre_lambda, epoch+1, dre_epochs, train_loss/(batch_idx+1), timeit.default_timer()-start_time))

        avg_train_loss.append(train_loss/(batch_idx+1))

        # save checkpoint
        if path_to_ckpt is not None and ((epoch+1) % 50 == 0 or (epoch+1)==dre_epochs):
            ckpt_fullpath = path_to_ckpt + "/DRE_{}_checkpoint_epoch_{}.pth".format(dre_net_name, epoch+1)
            torch.save({
                    'epoch': epoch,
            ...
            # save loss
            logfile_fullpath = path_to_ckpt + "/DRE_{}_train_loss_epoch_{}.npz".format(dre_net_name, epoch+1)
