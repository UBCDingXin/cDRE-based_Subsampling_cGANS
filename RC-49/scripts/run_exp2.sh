...
############################################
# EXP 2: NImgPerLabel=15

ROOT_PATH="./RC-49"
DATA_PATH="./RC-49/data/RC-49_64x64.h5"

NCPU=8
MAX_N_IMG_PER_LABEL=15
...
echo "-------------------------------------------------------------------------------------------------"
echo "CcGAN: ${CcGAN_type}; No Subsampling"
...
DRE_type="hard"
echo "-------------------------------------------------------------------------------------------------"
echo "CcGAN: ${CcGAN_type}; Subsampling: conditional, ${DRE_type}, ${DRE_kappa}; Filtering: Yes"
CUDA_VISIBLE_DEVICES=1,0 python3 main.py \
--exp_name $EXP_NAME --root_path $ROOT_PATH --data_path $DATA_PATH --num_workers $NCPU \
--max_num_img_per_label $MAX_N_IMG_PER_LABEL \
--kernel_sigma -1 --threshold_type $CcGAN_type --kappa -2 \
