...
from train_filter_cnn import test_filter_cnn
...
def train_ccgan(kernel_sigma, kappa, train_images, train_labels, netG, netD, net_y2h, save_images_folder, save_models_folder = None, clip_label=False):
    ...
    for niter in range(resume_niters, niters):
        ...
        train_dataloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size_disc, shuffle=False, num_workers=NCPU)
...
def SampCcGAN_given_labels(netG, net_y2h, labels, batch_size = 500, to_numpy=True, verbose=True, PreNetFilter=None, filter_mae_cutoff_point=1e30, num_workers=0):
    '''
    ...
    with torch.no_grad():
        ...
        while n_img_got < nfake:
            ...
            if PreNetFilter is not None:
                ...
                batch_dataset = IMGs_dataset(batch_fake_images.cpu().numpy(), y.cpu().numpy(), normalize=False)
                batch_loader = torch.utils.data.DataLoader(batch_dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers)
                _, batch_mae_loss = test_filter_cnn(net=PreNetFilter, testloader=batch_loader, verbose=False)
                indx_accept = np.where(batch_mae_loss<=filter_mae_cutoff_point)[0]
                batch_fake_images = batch_fake_images[indx_accept]
                fake_labels_filtered.append(y[indx_accept].cpu())
    ...
    if PreNetFilter is not None:
        fake_labels = (torch.cat(fake_labels_filtered, dim=0)).numpy()
