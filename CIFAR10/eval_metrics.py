...
def compute_FID(PreNetFID, IMGSr, IMGSg, batch_size = 500, resize = None):
    ...
    with torch.no_grad():
        test_img = torch.from_numpy(IMGSr[0].reshape((1,IMGSr.shape[1],IMGSr.shape[2],IMGSr.shape[3]))).type(torch.float).cuda()
...
# Inception Scores
##############################################################################
def compute_IS(PreNetIS, IMGSg, batch_size = 500, splits=1, resize = None):
    #resize: if None, do not resize; if resize = (H,W), resize images to 3 x H x W
    ...
    with torch.no_grad():
        test_img = torch.from_numpy(IMGSg[0].reshape((1,IMGSr.shape[1],IMGSr.shape[2],IMGSr.shape[3]))).type(torch.float).cuda()
        if resize is not None:
            test_img = nn.functional.interpolate(test_img, size = resize, scale_factor=None, mode='bilinear', align_corners=False)
